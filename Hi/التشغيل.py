...
async def play(client, m: Message):
    ...
    if replied:
        if replied.audio or replied.voice:
            await m.delete()
            huehue = await replied.reply("Being openra tional and processd...")
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_video(
                    video="https://telegra.ph/file/fdfe5d1cdbb0af75c41e6.jpg",
                    caption=f"""
song  : [{songname}]({link})
id : {chat_id}
...
@Client.on_message(filters.command(["اغنيه_ع"], prefixes=f"{HNDLR}"))
...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            await m.reply("ماكو شي اتخطا !")
        elif op == 1:
            await m.reply("قائمة انتظار فارغه ، ثيملا")
        else:
            await m.reply(                f" التشغيل هسه - [{op[0]}]({op[1]}) | `{op[2]}`",                disable_web_page_preview=True,            )
    else:
...
async def pause(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(                f" تم اوكف الايقاف التشغيل  مؤقتًا.\n\n• تكدر ترجع التشغيل دز امر   `{HNDLR}ايقاف_الاستئناف`"            )
        except Exception as e:
            await m.reply(f"اكو خطأ ** \n`{e}`")
    else:
        await m.reply("**  There is no working song. !**") 
