...
@Client.on_message(filters.command(["ن", "بحث"], prefixes=f"{HNDLR}"))
async def song(client, message: Message):
    urlissed = get_text(message)
    if not urlissed:
        await client.send_message(            message.chat.id,            "صيغة الأمر غير صالحة .",        )
        return
    pablo = await client.send_message(message.chat.id, f"@kzzgg جاي يحمل انتضر `{urlissed}`")
...
async def progress(current, total, message, start, type_of_ps, file_name=None):
    ...
    if round(diff % 10.00) == 0 or current == total:
        ...
        progress_str = "{0}{1} {2}%\n".format(            "".join("ً" for i in range(math.floor(percentage / 10))),            "".join("" for i in range(10 - math.floor(percentage / 10))),            round(percentage, 2),        )

        tmp = progress_str + "{0} of {1}\n𝐄𝐓𝐀 : {2}".format(            humanbytes(current), humanbytes(total), time_formatter(estimated_total_time)        )
        if file_name:
            try:
...
@Client.on_message(filters.command(["بحث_فيد", "حمل_فيد"], prefixes=f"{HNDLR}"))
async def vsong(client, message: Message):
    urlissed = get_text(message)

    pablo = await client.send_message(message.chat.id, f" يبحث  `{urlissed}`")
    if not urlissed:
        await pablo.edit(            "The command format is invalid!"        )
        return
    ...
    capy = f"""
from : {m.from_user.mention} .{message.from_user.mention}
"""
